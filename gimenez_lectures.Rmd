...
```{r, out.width = '10cm',out.height='6cm',echo=FALSE}
...
```{r}
R2jags::traceplot(reglogcig.sample,mfrow = c(1, 2),
...
```{r echo=FALSE, message=FALSE, warning=FALSE, fig.asp=0.55}
...
8. Examine residuals: check out our favorite textbook and the [vignette](https://cran.r-project.org/web/packages/DHARMa/vignettes/DHARMa.html) of the `DHARMa` package.
...
n = length(y)
```
```{r, echo=FALSE, fig.width = 6, fig.height = 8, fig.asp = 0.5}
# graphical representation
library(lattice)
dat = data.frame(Biomass=x,nb_grain=y,Species=Sp)
xyplot(nb_grain ~ Biomass | Species,data=dat,xlab='Biomass',ylab='Number of grains')
```
...
Fit linear \alert{mixed} model, with species as a random effect on the \alert{intercept}
...
Fit linear \alert{mixed} model, with species as random effect on both \alert{intercept} and \alert{slope}
...
```{r, echo=TRUE, message=FALSE, warning=FALSE}
allom.lmm3 <- lmer (nb_grain ~ Biomass + (1 | Species) + 
                                   (0 + Biomass | Species), dat)
allom.lmm3
```
...
```{r, echo=FALSE, fig.asp = 0.6}
df_zoom <- df_models %>% 
  filter(Species %in% c("AVEBAR", "EROCIC", "ARIROT", "CONSUM"))
...
```{r, echo=FALSE, message=FALSE, warning=FALSE,fig.align='center',out.width='10cm',out.height='6cm'}
dotplot(ranef(m,condVar=TRUE))
```

## Calculate proportion of explained variance in GLMMs

```{r, out.width = '8cm',out.height='10cm',echo=FALSE}
...
## Weight of offspring per female as a function of gestation duration

```{r, echo=FALSE, fig.asp = 0.6}
library(lattice)
xyplot(offspring_weight ~ gestation | female_id,data=dat,xlab='duration of gestation (days)',ylab='offspring weight (g)')
#plot(dat$GESTATION,dat$POIDS,xlab='gestation',ylab='poids')
library(lme4)
```

## Weight of offspring per year as a function of gestation duration

```{r, echo=FALSE, fig.asp = 0.6}
library(lattice)
xyplot(offspring_weight ~ gestation | as.factor(year),data=dat,xlab='duration of gestation (days)',ylab='offspring weight (g)')
#plot(dat$GESTATION,dat$POIDS,xlab='gestation',ylab='poids')
library(lme4)
```
...
par(mfrow=c(1,2))
plot(data2025$gestation,data2025$offspring_weight,xlab='gestation',ylab='offspring_weight',main='2010',ylim=c(-200,700))
...
plot(data2026$gestation,data2026$offspring_weight,xlab='gestation',ylab='offspring_weight',main='2011',ylim=c(-200,700))
...
- We can \alert{generalize to a wider population}
      - e.g. can say something about the allometry relationship that we might expect in the population of species from which our sample was selected (plant example)
      
- \alert{Fewer parameters} are needed than separate linear regressions per species
      - we only needed few additional parameters – e.g. the variance to allow the intercepts to vary from species to species. By contrast, the standard species by species approach would require 33 additional parameters (plant example)

## Mixed models in a nutshell

- Information can be \alert{shared between groups}
      - By assuming that the random effects come from a common distribution, a multilevel model can share information between groups. This can improve the precision of predictions for species / individual that have little data

- Possible extension to the \alert{inclusion of predictors at the group level}
      - e.g. if our data included say functional traits for species (herbaceous), we could include a term in the species level model to describe the difference in number of seeds according to functional characteristics (plant example)
